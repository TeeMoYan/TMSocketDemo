...
- (IBAction)sendMessage:(UIButton *)sender {
    ...
    ssize_t send_length = send(self.mSocketId, send_msg, strlen(send_msg), 0);
...
- (void)connectSocket{
    ...
    int result = connect(self.mSocketId,(const struct sockaddr *)&socket_add, sizeof(socket_add));
...
- (void)receiveMessage{
    ...
    while (1) {
        ...
        uint8_t buffer[1024];
        ssize_t receive_length =  recv(self.mSocketId, buffer, sizeof(buffer), 0);
        NSLog(@"接收到了========%ld字节==========",receive_length);
        if (receive_length <= 0) {
            return;
        }
        
        NSData *data = [NSData dataWithBytes:buffer length:receive_length];
        NSString *str = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"接收到了==================%@",str);
        dispatch_async(dispatch_get_main_queue(), ^{
            [self showMessage:str msgType:1];
        });
    }
}
